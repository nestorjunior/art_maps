defmodule ArtMapsWeb.UserAuthTest do
  use ArtMapsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias ArtMaps.Accounts
  alias ArtMaps.Accounts.Scope
  alias ArtMapsWeb.UserAuth

  import ArtMaps.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ArtMapsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ArtMapsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ArtMapsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
